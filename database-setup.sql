...
-- Website content policies (public read, admin write)
...
-- Version history policies (admin only)
CREATE POLICY "Admins can access version history" ON version_history
    FOR ALL USING (auth.role() = 'authenticated');

-- Media content policies (public read, admin write)
...
-- Step 6: Insert Initial Website Content
